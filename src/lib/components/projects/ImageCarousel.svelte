  ...
  <div class="relative rounded-xl overflow-hidden shadow-lg bg-white">
    ...
    <div class="relative h-[300px] sm:h-[400px] md:h-[500px] w-full">
      ...
      {#each images as image, index}
        <div 
          ...
          style="opacity: {currentIndex === index ? '1' : '0'}; z-index: {currentIndex === index ? '10' : '0'};"
        >
          <img 
    ...
    <div class="absolute bottom-4 left-0 right-0 flex justify-center space-x-2 z-20">
      {#each images as _, index}
        <button 
          class="h-2 rounded-full transition-all {currentIndex === index ? 'w-8 bg-primary-600' : 'w-2 bg-gray-300 hover:bg-primary-400'}" 
